...
export default async function placeOrder(req: Request, res: Response) {
    try {
        ...
        if (allPricesSummed) {
            ...
            res.status(400);
            res.json({
                msg: `Some products in your order were not found. Product IDs: ${Object.keys(missingProducts).join(', ')
                    }. Please emtpy the cart and retry.`,
            });
            res.end();
            return;
        }
        ...
        const collect = {
            body: {
                intent: "CAPTURE" as CheckoutPaymentIntent.CAPTURE,
                purchaseUnits: [
                    {
                        amount: {
                            currencyCode: "EUR",
