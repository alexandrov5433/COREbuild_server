...
export default async function updateProductSpecsDoc(req: Request, res: Response) {
    try {
        const newSpecsDocFile = req.files.specsDoc as UploadedFile || null;
        if (!newSpecsDocFile) {
        ...
        const newProductData = await updateProductSpecsDocRefInDB((newSpecsDocFileData as FileData).fileID, productID);
        if (!newProductData) {
